...
def exportPngs(voxels, bounding_box, outputFilePath):
    size = str(len(str(bounding_box[2])))
    outputFilePattern, outputFileExtension = os.path.splitext(outputFilePath)
    for height in range(bounding_box[2]):
...
def exportSvx(voxels, bounding_box, outputFilePath, scale, shift):
    ...
    root = ET.Element("grid", attrib={"gridSizeX": str(bounding_box[0]), #No of voxels in X direction
                                      "gridSizeY": str(bounding_box[2]), #No of voxels in Y direction
                                      "gridSizeZ": str(bounding_box[1]), #No of voxels in Z direction
                                      "voxelSize": str(1.0/scale[0]/1000), #STL is probably in mm, and svx needs meters
                                      "subvoxelBits": "8",        # number of bits used per voxel for density (1-16), default is 8.
                                      "originX": str(-shift[0]),
                                      "originY": str(-shift[2]),
                                      "originZ": str(-shift[1]),
                                      })
